...
export async function getScheduledEmails(
  token: string
): Promise<ApiResponse<{ scheduledEmails: ScheduledEmail[]; total: number }>> {
  const client = new ApiClient(API_BASE_URL);
  client.setToken(token);
  return client["request"]('/api/email/scheduled', {
    method: 'GET'
  });
}
...
export async function deleteScheduledEmail(
  ...
  const client = new ApiClient(API_BASE_URL);
  client.setToken(token);
  return client["request"](`/api/email/scheduled/${emailId}`, {
    method: 'DELETE'
  });
}
...
): Promise<ApiResponse<{ success: boolean; scheduledEmail: ScheduledEmail }>> {
  const client = new ApiClient(API_BASE_URL);
  client.setToken(token);
  return client["request"](`/api/email/scheduled/${emailId}`, {
    method: 'PUT',
